 ...
 *
 *   - "6 pods" = six `TenantRateLimiter` instances. When they share one Redis
 *     (production), routing a tenant's requests round-robin across all six
 ...
 *     pod hands the tenant a fresh bucket, so far more requests are wrongly
 *     allowed.
 *
...
import { TenantRateLimiter } from '../../src/api/middleware/rate_limiter.js';
...
function makePod(store: Map<string, BucketState>): TenantRateLimiter {
  return new TenantRateLimiter(new FakeBucketRedis(store) as unknown as Redis);
}
...
async function runSequence(
  ...
  for (const req of requests) {
    ...
    const result = await pod.checkLimit(profile(req.tenantId));
...
describe('rate limiter HPA scale-up consistency (issue #50)', () => {
  ...
  // Deterministic, routing-order-independent request list: each tenant fires a
  // BURST_PER_TENANT burst, interleaved tenant-by-tenant so the round-robin pod
  // assignment (index % POD_COUNT) spreads each tenant's events across pods.
  function buildBurstRequests(tenants: number): { tenantId: string }[] {
    const reqs: { tenantId: string }[] = [];
    for (let r = 0; r < BURST_PER_TENANT; r++) {
      for (let d = 0; d < tenants; d++) {
        reqs.push({ tenantId: `tenant-${String(d)}` });
      }
    }
    return reqs;
  }
  ...
  it('negative control: 6 pods with isolated state drift far beyond 1%', async () => {
    ...
    // Old bug: each pod keeps its own bucket, so a tenant routed round-robin
