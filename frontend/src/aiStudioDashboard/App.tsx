...
import type { DePinNode, RetryQueueTransaction, TelemetryPayload, VariableTariff } from './types';
...
export default function DashboardSimulatorApp() {
  ...
  const generateTelemetryForNode = useCallback((node: DePinNode) => {
  ...
  }, [isContractPaused, isNetworkOnline, tariffs]);
  ...
  }, [generateTelemetryForNode, nodes]);
  ...
  const handleForceSync = () => {
    ...
    let delay = 0;
    bufferedTransactions.forEach((tx) => {
      // Mark as syncing
