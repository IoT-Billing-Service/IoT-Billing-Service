...
<!-- Appendix consolidated from backend/contracts/price_oracle_client/README.md (issue #308) -->

## price_oracle_client JS

JS library for interacting with [Soroban](https://soroban.stellar.org/) smart contract `price_oracle_client` via Soroban RPC.
...
soroban contract bindings ts \
  ...
  --contract-id CD7QY4BY3OFNVNZGWFZ2YUCILFNGUP4ZZUGU7KBUOE2P3ZUDCHA3AKCI \
  --output-dir ./path/to/price_oracle_client
```
...
```json
"dependencies": {
  "price_oracle_client": "./path/to/this/folder"
}
```
...
```json
"scripts": {
  "postinstall": "soroban contract bindings ts --rpc-url http://localhost:8000/soroban/rpc --network-passphrase \"Standalone Network ; February 2017\" --id CD7QY4BY3OFNVNZGWFZ2YUCILFNGUP4ZZUGU7KBUOE2P3ZUDCHA3AKCI --name price_oracle_client"
}
```
...
```js
import { Contract, networks } from "price_oracle_client"
